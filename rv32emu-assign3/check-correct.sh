...
do             
    ...
    do
        ...
        do
            ...
            echo "[+] Compare: [./$NATIVE_PROG] vs. [./emu-rv32i-dir-cache ./$EMU_PROG]"
            # diff <(./$NATIVE_PROG) <(./emu-rv32i-$cache_type-cache ./$EMU_PROG|awk /./|grep -a -v ">>>") >/dev/null
            diff <(./$NATIVE_PROG) <(./emu-rv32i-dir-cache ./$EMU_PROG|awk /./|grep -a -v ">>>") >/dev/null
            if [ $? != 0 ]
            then
                # echo "[-] FAIL: Execution results seem to be wrong for [./emu-rv32i-$cache_type-cache $EMU_PROG]."
                echo "[-] FAIL: Execution results seem to be wrong for [./emu-rv32i-dir-cache $EMU_PROG]."
                exit -1
            fi
        done
    done
